...
for src in ${all_srcs[@]}; do
    sed -i -e 's@#include <hipblas.h>@#include <hipblas/hipblas.h>@' \
        ...
        -e 's@#include <cub/cub.cuh>@#include <hipcub/hipcub.hpp>@' `# TODO: File issue` \
        -e 's@#include "hip/extension/@#include "./cuda/extension/@' `# TODO: File issue` \
        -e 's@#include "hip/cooperative_minibatching_utils.h"@#include "./cuda/cooperative_minibatching_utils.h"@' `# TODO: File issue` \
        -e 's@#include "hip/max_uva_threads.h"@#include "./cuda/max_uva_threads.h"@' `# TODO: File issue` \
        -e 's@\.\./hip/@@' `# TODO: File issue` \
        -e 's@\bDeviceFor::Bulk\b@Bulk@g' \
        -e 's@\bcub::@hipcub::@g' `# TODO: File issue` \
        -e 's@\bcuda::stream_ref@cuco::cuda_stream_ref@' `# TODO: File issue` \
        -e 's@\bcuda::::min@min@' `# TODO: File issue` \
        -e 's@\bcuda::proclaim_return_type@proclaim_return_type@' `# TODO: File issue` \
        -e 's@\bDGL_USE_CUDA\b@DGL_USE_ROCM@g' \
        -e 's@\b__CUDA_ARCH__\b@__HIP_DEVICE_COMPILE__@g' `# TODO file issue` \
        ...
        -e 's@\bnv_bfloat16\b@hip_bfloat16@g' `# TODO: File issue` \
        -e 's@\b__trap();@abort();@' \
