...
@pytest.mark.parametrize("rank", list(range(1,2)))
def test_rank_sort_and_unique_and_compact(dtype, rank):
    torch.manual_seed(7)
    nodes_list1 = [
        torch.randint(0, 2111, [77], dtype=dtype, device=F.ctx())
        for _ in range(10)
    ]
...
# 
# nodes_list1 = [
#     torch.randint(0, 2111, [777], dtype=torch.int64, device=F.ctx())
#     for _ in range(10)
# ]
# res1 = torch.ops.graphbolt.rank_sort(nodes_list1, 2, 7)
