...
// TODO(tpopp): CUDA_ARCH->Checking hip atomics
#if defined(__HIP_ARCH_HAS_GLOBAL_INT32_ATOMICS__) \
  ...
  #define __HIP_ATOMICS__ 1
#else
  #define __HIP_ATOMICS__ 0
#endif
#if __HIP_ATOMICS__
// TODO(tpopp): Added for atomic_ref
#include <hip/atomic>
...
c10::intrusive_ptr<sampling::FusedSampledSubgraph> SampleNeighbors(
  ...
  AT_DISPATCH_INDEX_TYPES(
      indptr.scalar_type(), "SampleNeighborsIndptr", ([&] {
		...
		// TOOD(tpopp): manually corrected nv_bfloat16->hip_bfloat16
          ...
          const dim3 block(BLOCK_SIZE);
          const dim3 grid(
              (::min(num_edges.value(), static_cast<int64_t>(1 << 20)) +
               BLOCK_SIZE - 1) /
              BLOCK_SIZE);
          AT_DISPATCH_INDEX_TYPES(
              indices.scalar_type(), "SampleNeighborsIndices", ([&] {
