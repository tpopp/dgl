...
// TODO(tpopp): added to get proclaim_return_type
...
RankSortImpl(
  ...
  torch::Tensor unique_ids, ret_index, unique_ids_offsets;
        at::cuda::CUDAEvent unique_ids_offsets_event;
  // std::tie(unique_ids, ret_index, unique_ids_offsets, unique_ids_offsets_event) =
            ...
            [=, part_ids = part_ids_sorted.data_ptr<cuda::part_t>(),
             offsets = offsets.data_ptr<int64_t>()] __device__(int64_t i) {
              ...
              const auto offset_end =
                  offsets_dev_ptr[::min(batch_id + 1, num_batches)];
		  // TODO(tpopp): cub -> hipcub
              offsets[i] = hipcub::LowerBound(
...
std::vector<std::tuple<torch::Tensor, torch::Tensor, torch::Tensor>> RankSort(
  ...
  CUDA_CALL(hipMemcpyAsync(
      ...
      sizeof(int64_t) * offsets.numel(), hipMemcpyHostToDevice,
