...
// TODO(tpopp): hipcub.hpp missing device_for.hpp inclusion
...
// TODO(tpopp): Manually reverted change.
#include "../common.h"
#include "../utils.h"
...
std::tuple<torch::Tensor, torch::Tensor, int64_t, int64_t> GpuGraphCache::Query(
  ...
  return AT_DISPATCH_INDEX_TYPES(
	...
	// TODO(tpopp): manual structured binding
	auto tup = 
            static_cast<thrust::tuple<int64_t, int64_t, int64_t>>(
                num_threshold_new_hit_cpu);
        const auto [num_threshold, num_new, num_hit] = std::tie(thrust::get<0>(tup), thrust::get<1>(tup), thrust::get<2>(tup));
        map_size_ += num_new;
            ...
            indptr_.scalar_type(), "GpuGraphCache::Replace::copy_prep", ([&] {
              ...
              const auto missing_edge_ids =
                  edge_id_offsets ? edge_tensors.back().data_ptr<indptr_t>()
                                  : nullptr;
	      // TODO(tpopp): hipcub doesn't define DeviceFor
...

              {
                ...
                for (int64_t i = 0; i < num_buffers; i += max_copy_at_once) {
                  CUB_CALL(
                      ...
                      ::min(num_buffers - i, max_copy_at_once));
                }
              }
