  ...
  T Wait() {
    ...
    if constexpr (std::is_void_v<T>) {
      if (std::holds_alternative<T_with_event>(result)) {
        ...
        event.block(c10::hip::getCurrentHIPStreamMasqueradingAsCUDA());
      }
      return;
...
inline auto async(F&& function, bool is_cuda = false) {
  ...
  if (is_cuda) {
    stream_data = c10::hip::getCurrentHIPStreamMasqueradingAsCUDA().pack3();
  }
#endif
  ...
  auto fn = [=, func = std::move(function)]() -> return_type {
    ...
    // async operation.
    if (is_cuda) {
      auto stream = c10::hip::HIPStreamMasqueradingAsCUDA::unpack3(
          stream_data.stream_id, stream_data.device_index,
          stream_data.device_type);
      c10::hip::HIPStreamGuardMasqueradingAsCUDA guard(stream);
